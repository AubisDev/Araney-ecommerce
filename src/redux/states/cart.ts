  ...
  reducers: {
    addCartItem: (state, action) => {
      ...
      if (foundItem) {
        ...
        foundItem.amount =
          amountSum < maxItemAmount ? amountSum : maxItemAmount;
        return;
      }
    ...
    removeCartItem: (state, action) => {
      let newCartState = state.filter(
        ({ item }: CartInfo) => item.id !== action.payload
      );
      return newCartState;
    },

    increaseCartItemAmount: (state, action) => {
      let itemFound = state.find(
        (cartItem: CartInfo) => cartItem.item.id === action.payload
      );
